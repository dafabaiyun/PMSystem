    ...
    <div>
        ...
        <div class="table">
            <el-table :data="attendanceData" style="width: 100%">
                ...
                <el-table-column prop="appTime" label="申请时间" width="180" />
...
import { getStaffLeaveBySno, getStaffLeaveByStatus, insertStaffLeave, Leave, updateStaffLeave, updateStaffLeaveStatus } from '@/api/staffLeave';
...
async function pass(leaveNo, passFlag) {
    if (user.role === Role['部门主管']) {
        // 调用审核接口
        passFlag ? await updateStaffLeaveStatus({ leaveNo, appStatus: Leave['待审批'].toString() }) : await updateStaffLeaveStatus({ leaveNo, appStatus: Leave['未通过'].toString() })
    }
